...
struct keypair *init_rsa_keypair(const char *skname, const char *pkname)
{
  ...
  if (skname)
  {
    FILE *fp = fopen(skname, "rb");
    ret->priv = PEM_read_RSAPrivateKey(fp, NULL, NULL, NULL);
    fclose(fp);
  }

  if (pkname)
  {
    FILE *fp = fopen(pkname, "rb");
    ret->pub = PEM_read_RSA_PUBKEY(fp, NULL, NULL, NULL);
    fclose(fp);
  }
    ...
    unsigned char *output, int *olen)
{
  ...
  *olen = RSA_private_decrypt(ilen, input, output, kst->priv, RSA_PKCS1_OAEP_PADDING);
  ...
  ffinish("olen %d output %s ret: %d", olen, output,ret);
  return ret;
}
